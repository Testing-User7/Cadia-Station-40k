...
/obj/machinery/newscaster
	name = "newscaster"
	desc = "A standard Imperial-licensed newsfeed handler for use in commercial space stations. All the news you absolutely have no use for, in one place!"
		...
		switch(screen)
			if(0)
				...
				if(src.securityCaster)
					...
					dat+="<BR><A href='?src=\ref[src];menu_censor_channel=1'>Mark Feed Channel with Imperial D-Notice</A>"
			...
			if(9)
				...
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the outpost, and marked with a Imperial D-Notice.<BR>"
			...
			if(10)
				dat+="<B>Imperial Feed Censorship Tool</B><BR>"
				...
				dat+="<BR><A href='?src=\ref[src];setScreen=[0]'>Cancel</A>"
			if(11)
				dat+="<B>Imperial D-Notice Handler</B><HR>"
			...
			if(13)
				...
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with a Imperial D-Notice.<BR>"
					dat+="No further feed story additions are allowed while the D-Notice is in effect.</FONT><BR><BR>"
				else
					if( isemptylist(src.viewing_channel.messages) )
						dat+="<I>No feed messages found in channel...</I><BR>"
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if(href_list["set_channel_name"])
				...
				if(choice=="Confirm")
					if(input_param==1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one. See the else below
						...
						if(news_network.wanted_issue.is_admin_message)
							alert("The wanted issue has been distributed by a Imperial higherup. You cannot edit it.","Ok")
							return
			...
			if(news_network.wanted_issue.is_admin_message)
				alert("The wanted issue has been distributed by a Imperial higherup. You cannot take it down.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by a Imperial Officer. You cannot censor it.","Ok")
				return
			...
			if(MSG.is_admin_message)
				alert("This message was created by a Imperial Officer. You cannot censor its author.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by a Imperial Officer. You cannot place a D-Notice upon it.","Ok")
				return
...
/obj/item/weapon/newspaper
	name = "newspaper"
	desc = "An issue of The Griffon, the newspaper circulating aboard Imperial Outposts."
	...
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				...
				dat+="<DIV ALIGN='center'><FONT SIZE=2>Imperial-standard newspaper, for use on Imperial Outposts</FONT></div><HR>"
