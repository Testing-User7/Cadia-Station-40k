...
// Vomits everything you send it to the world. Useful for debugging hopefully.
/proc/debug_stuff()
	for(var/e in args)
		world << e
	return
...
// Clone of pick()
/proc/n_pick() //Who wrote these procs... This didn't even used to work for a nested list for emperor's sake. -Drake
		...
		if(isobject(e))
			if(istype(e, /list))
				...
				for(var/sube in sublist)
					finalpick.Add(sube)
				continue
		*/ //...and no. This bit of code is not good for any respectable interpreter.
		finalpick.Add(e)
	if(finalpick.len) //Who on earth coded a scripting interface nondefensively? No player written code should be able to throw actual server runtime errors just because it has an error in it. -Drake
		world << "picked [finalpick]"
		return pick(finalpick)
	else
		world << "pick returned null"
		return null //Lol if it isn't yet apparent I don't like the way this proc was written at all.
...
/proc/n_listpos(var/list/L, var/pos, var/value)
	...
	if(isnum(pos))
		...
		if(!value)
			world << "returning [L[pos]] from at"
			return L[pos]
		else
			L[pos] = value
	world << "at did not return anything"
