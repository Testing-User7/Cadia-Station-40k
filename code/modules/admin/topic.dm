/datum/admins/Topic(href, href_list)
	...
	if(href_list["makeAntag"])															//These all call the one_click_antag
		switch(href_list["makeAntag"])
			...
			if("7")
				...
				if(!src.makeNukeTeam())
					usr << "\red Unfortunatly there were no candidates available"
			if("8")
				message_admins("[key_name(usr)] spawned a ninja.")						//not connected to anything
				log_admin("[key_name(usr)] spawned a ninja.")
				src.makeSpaceNinja()
			if("9")
				message_admins("[key_name(usr)] started an alien infestation.")			//Not connected to anything
				log_admin("[key_name(usr)] started an alien infestation.")
				src.makeAliens()
			if("10")
				message_admins("[key_name(usr)] Turned a bunch of ghosts into Sisters of Battle.")
				log_admin("[key_name(usr)] summoned SOB.")
				src.makeSOBsquad()
			if("11")
				message_admins("[key_name(usr)] Turned a bunch of ghosts into Tau.")
				log_admin("[key_name(usr)] summoned Tau.")
				src.makeTAUsquad()
			if("12")
				message_admins("[key_name(usr)] Turned a bunch of ghosts into Ultramarines.")
				log_admin("[key_name(usr)] summoned Ultramarines.")
				src.makeUMsquad()
			if("13")
...
/*				E = new /datum/round_event/weightless()
			...
			if("eagles")//SCRAW
				...
				priority_announce("Segmentum airlock control override activated. Please take this time to get acquainted with your coworkers.", null, 'sound/AI/commandreport.ogg')
			if("guns")
				feedback_inc("admin_secrets_fun_used",1)
